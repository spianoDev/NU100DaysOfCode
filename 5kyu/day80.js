...
// id of the process that created it, and so on. More information can be found in the MongoDB documentation
// (which you have to read in order to implement this Kata).
//
...
let Mongo = {
    'isValid' : function(s){
        if (s.length !== 24) {
            ...
            console.log(ObjectID.isValid(s));
    ...
    'getTimestamp' : function(s){
        if (typeof s !== 'string') {
            ...
            console.log(new Date(parseInt(s.substring(0, 8), 16) * 1000).toString());
            return new Date(parseInt(s.substring(0, 8), 16) * 1000).toString();
        }
    }
};
/** Advanced Solution (Refactored) **/

// Above solution did not quite work, but I couldn't figure out what this one was looking for to fix it.
...
// Mongo.getTimestamp('507f1f77bcf86cz799439011'); // => false
// Mongo.getTimestamp('507f1f77bcf86cd79943901'); // => false
Mongo.getTimestamp('111111111111111111111111'); // => Sun Jan 28 1979 00:25:53 GMT-0800 (Pacific Standard
// Time)
// Mongo.getTimestamp(111111111111111111111111);  // => false
